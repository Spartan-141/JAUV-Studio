...
ipcMain.handle('reportes:resumen', async (_e, { fecha_desde, fecha_hasta }) => {
  const db = getDb();
  ...
  // Recent sales list
  const listaVentas = await db.all(`
...
ipcMain.handle('reportes:cierre_caja', async (_e, fecha) => {
  const db = getDb();
  const desde = fecha + ' 00:00:00';
  const hasta = fecha + ' 23:59:59';

  const pagosDia = await db.all(`
