...
// Database path on Linux usually follows Electron's appData/productName pattern
// For this app, it's ~/.config/jauv-studio-pos/jauv_pos.db
const dbPath = path.join(os.homedir(), '.config', 'jauv-studio-pos', 'jauv_pos.db');
...
// Function to generate a random code if not provided (though handled by IPC usually)
...
const tx = db.transaction(() => {
  for (const p of products) {
    ...
    insert.run(code, p.nombre, p.marca, p.compra, p.venta, p.stock, p.min, p.cat, p.desc);
    count++;
  }
});
