...
import { LuTrendingUp, LuHistory, LuPackage, LuBanknote, LuSmartphone, LuLandmark, LuChevronDown, LuChevronUp, LuCreditCard, LuCalendarDays, LuLock, LuCalendarClock, LuRefreshCw, LuCircleCheck, LuChartColumn, LuChevronLeft, LuChevronRight, LuListOrdered } from 'react-icons/lu'
...
function MetodosTable({ pagos, abonos }) {
  const combined = {}
  ;[...pagos, ...abonos].forEach(({ metodo, total_ves }) => {
    if (!combined[metodo]) combined[metodo] = 0
    combined[metodo] += total_ves || 0
  })
    ...
    <div className="card p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {Object.entries(METODO_LABEL).map(([key, label]) => {
          const total = combined[key] || 0
          return (
            <div key={key} className="bg-surface-700 rounded-xl p-4">
              <p className="text-xs text-gray-400 mb-1">{label}</p>
              <p className="text-lg font-bold text-white">Bs. {Number(total).toLocaleString('es-VE', { maximumFractionDigits: 2 })}</p>
            </div>
          )
        })}
      </div>
  ...
  return (
    <>
      <tr className="cursor-pointer hover:bg-surface-700/50" onClick={() => setOpen(o => !o)}>
        ...
        <td className="text-right font-semibold text-white">{fmt(v.total_usd || v.total || 0)}</td>
        <td className="text-right text-accent-green">{fmt(pagado)}</td>
        <td className="text-right">{(v.saldo_pendiente_usd || v.saldo_pendiente) > 0
          ? <span className="text-red-400">{fmt(v.saldo_pendiente_usd || v.saldo_pendiente)}</span>
                ...
                <tbody>
                  {(v.detalles || []).map((d, i) => (
                    <tr key={i} className="border-t border-white/5">
                      ...
                      <td className="text-right text-gray-400">{fmt(d.precio_unitario_usd || d.precio_unitario || 0)}</td>
                      <td className="text-right text-white font-medium">{fmt(d.subtotal_usd || d.subtotal || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <div className="mt-2 pt-2 border-t border-white/5 text-xs text-gray-500 space-y-0.5">
                  ...
                  {[...(v.pagos || []), ...(v.abonos || [])].map((p, i) => (
                    <div key={i} className="flex justify-between">
                      ...
                      <span className="text-accent-green">{fmt(p.monto_ves || p.monto || 0)}</span>
                    </div>
                  ))}
                </div>
              )}
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        ...
        <StatCard
          label="Ingresos"
          value={fmt(data.ingresos || data.ingresos_usd || 0)}
          green
        />
        <StatCard
          label="Ganancia Neta"
          value={fmt(data.ganancia_neta || data.ganancia_neta_usd || 0)}
          green
        />
        <StatCard
          label="Descuentos"
          value={fmt(data.descuentos || data.descuentos_usd || 0)}
        />
        <StatCard
          label="Pendiente Cobrar"
          value={fmt(data.pendiente_cobrar || data.pendiente_cobrar_usd || 0)}
          red={(data.pendiente_cobrar || data.pendiente_cobrar_usd || 0) > 0}
        />
      </div>
...
// ── Inventario panel ──────────────────────────────────────────────────────────
function InventarioPanel({ reporte, fmt }) {
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={fmt(stats.inversion || stats.inversion_usd || 0)}
        />
        <StatCard
          label="Ganancia Potencial"
          value={fmt(stats.ganancia_potencial || stats.ganancia_potencial_usd || 0)}
          green
        />
      </div>
      ...
      <div className="card table-wrapper mt-4">
        ...
        <table>
          <thead><tr><th>Código</th><th>Producto</th><th className="text-center">Min. Sugerido</th><th className="text-center">Stock Actual</th></tr></thead>
          <tbody>
            {bajo_stock.length === 0 ? (
...
function VentasHistorialPanel({ fmt }) {
  ...
  const totalIngresos = data.ventas.reduce((s, v) => s + (v.total || v.total_usd || 0), 0)
  ...
  return (
    <div className="space-y-4">
      <div className="card p-4 flex flex-wrap gap-3 items-end">
        ...
        {mode === 'day' && <input type="date" className="input w-44" value={day} onChange={e => { setDay(e.target.value); setPage(1) }} />}
        {mode === 'month' && <input type="month" className="input w-44" value={month} onChange={e => { setMonth(e.target.value); setPage(1) }} />}
        {mode === 'range' && (
          <>
            <div className="flex items-center gap-2"><span className="text-xs text-gray-500">Desde</span><input type="date" className="input w-40" value={desde} onChange={e => { setDesde(e.target.value); setPage(1) }} /></div>
            <div className="flex items-center gap-2"><span className="text-xs text-gray-500">Hasta</span><input type="date" className="input w-40" value={hasta} onChange={e => { setHasta(e.target.value); setPage(1) }} /></div>
          </>
        )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="stat-card"><p className="stat-label">Ventas encontradas</p><p className="text-xl font-bold text-white">{data.total}</p></div>
        <div className="stat-card"><p className="stat-label">Ingresos (esta página)</p><p className="text-xl font-bold text-accent-green">{fmt(totalIngresos)}</p></div>
        <div className="stat-card hidden sm:block"><p className="stat-label">Registros por página</p><p className="text-xl font-bold text-white">{PER_PAGE}</p></div>
      </div>
      ...
      <div className="card table-wrapper">
        <table>
          <thead><tr>
            ...
            <th className="text-right">Total</th><th className="text-right">Cobrado</th>
            <th className="text-right">Pendiente</th><th></th>
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={8} className="text-center py-10 text-gray-500">Cargando ventas...</td></tr>
              : data.ventas.length === 0 ? <tr><td colSpan={8} className="text-center py-10 text-gray-500">No hay ventas para los filtros seleccionados</td></tr>
              : data.ventas.map(v => <VentaRow key={v.id} v={v} fmt={fmt} />)
            }
          </tbody>
        </table>
      ...
      {data.pages > 1 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-500">Página {data.page} de {data.pages} · {data.total} registros</span>
          <div className="flex gap-1">
            ...
            {Array.from({ length: Math.min(5, data.pages) }, (_, i) => {
              ...
              const p = start + i
              return <button key={p} onClick={() => load(p)} className={`btn-sm px-3 rounded-lg text-sm ${p === page ? 'bg-brand-600 text-white' : 'btn-ghost'}`}>{p}</button>
            })}
...
export default function Reportes() {
  ...
  const loadInventario = useCallback(async () => {
    setLoadingInv(true)
    const data = await window.api.invoke('reportes:inventario')
    setInventario(data)
    setLoadingInv(false)
  }, [])
  ...
  const handleCerrarDia = async () => {
    setCerrando(true)
    const res = await window.api.invoke('reportes:cerrar_dia')
    setCerrando(false)
    setShowConfirmCerrar(false)
    if (res?.error) {
      setAlertMsg(res.error)
    } else {
      await loadHoy(); await loadHistorial()
    }
  }
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-gray-500 capitalize">{fechaHoyLabel}</p>
        </div>
        {activeTab === 'hoy' && hoy && (
      ...
      <div className="shrink-0 bg-surface-800 rounded-xl p-1 border border-white/5 mb-5 overflow-x-auto">
        <div className="flex gap-1 min-w-max">
          {[
            { id: 'hoy', label: 'Reporte del Día', icon: <LuChartColumn /> },
            { id: 'historial', label: 'Historial Cerrado', icon: <LuCalendarClock />, badge: historial.length },
            { id: 'inventario', label: 'Inventario', icon: <LuPackage /> },
            { id: 'ventas', label: 'Todas las Ventas', icon: <LuListOrdered /> },
          ].map(t => (
            <button key={t.id} onClick={() => {
              setActiveTab(t.id)
              if (t.id === 'historial') loadHistorial()
              if (t.id === 'inventario') loadInventario()
            }}
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === t.id ? 'bg-brand-600 text-white shadow-glow' : 'text-gray-400 hover:text-white hover:bg-surface-700/50'}`}>
              {t.icon} {t.label}
              {t.badge > 0 && <span className="bg-white/10 rounded-full px-2 text-xs">{t.badge}</span>}
            </button>
          ))}
        </div>
      </div>
        ...
        <div>
          {hoy?.cerrado && (
            <div className="flex items-center gap-2 bg-surface-700/50 border border-white/10 rounded-xl px-4 py-2.5 mb-4 text-sm text-gray-400">
              ...
              <span>Último cierre guardado el <strong className="text-white">{hoy?.cerrado_en?.slice(0, 16)}</strong>. Si llegaron ventas después, usa <strong className="text-white">Actualizar Cierre</strong>.</span>
            </div>
          )}
          {loading ? (
            <div className="card-p text-center py-12 text-gray-500">Cargando datos de hoy...</div>
          ) : hoy ? (
            <DayPanel data={hoy} fmt={fmt} />
          ) : null}
        </div>
      )}
      ...
      {activeTab === 'historial' && (
        <div className="card p-5">
          ...
          {historial.length === 0 ? (
            ...
            <>
              <select className="select mb-5" value={selectedFecha} onChange={e => handleSelectFecha(e.target.value)}>
                <option value="">— Selecciona un día para ver su resumen —</option>
                {historial.map(h => (
                  <option key={h.fecha} value={h.fecha}>
                    {h.fecha} · {fmt(h.ingresos || 0)} · {h.total_ventas} ventas
                  </option>
                ))}
              </select>
              ...
              {detalle && !loadingDetalle && (
                <div className="border-t border-white/5 pt-5">
                  <p className="text-xs text-gray-500 mb-4">Cierre grabado el {detalle.cerrado_en?.slice(0, 16)}</p>
                  <DayPanel data={detalle} fmt={fmt} />
                </div>
              )}
            </>
          )}
      ...
      {activeTab === 'inventario' && (
        <div>
          ...
          {loadingInv ? (
            ...
            <InventarioPanel reporte={inventario} fmt={fmt} />
          ) : null}
        </div>
      )}

      {activeTab === 'ventas' && <VentasHistorialPanel fmt={fmt} />}

      {showConfirmCerrar && (
        <ConfirmationModal
          title={hoy?.cerrado ? '¿Actualizar el cierre?' : '¿Cerrar el día?'}
          message={hoy?.cerrado
            ? `Se actualizará el resumen guardado de hoy (${fechaHoy}) incluyendo las ventas del último momento.`
            : `Se guardará el resumen de hoy (${fechaHoy}).`
          }
          type="warning"
