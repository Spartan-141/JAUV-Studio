    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-lg" style={{ maxHeight: '88vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="flex items-start justify-between mb-5 shrink-0">
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-widest mb-1">Detalle de Venta</p>
            ...
            <p className="text-xs text-gray-500 mt-1">{v.fecha?.slice(0, 16)} &middot; {v.cliente_nombre || 'Sin cliente'}</p>
          </div>
          <button onClick={onClose} className="btn-ghost btn-sm text-xl text-gray-400 hover:text-white p-2">✕</button>
        </div>

        {/* Resumen financiero */}
        <div className="grid grid-cols-3 gap-3 mb-5 shrink-0">
          <div className="bg-surface-700/60 border border-white/5 rounded-xl p-3">
            <p className="text-xs text-gray-400 mb-1">Total Factura</p>
            <p className="text-lg font-bold text-white">{fmt(v.total)}</p>
          </div>
          <div className="bg-surface-700/60 border border-accent-green/20 rounded-xl p-3">
            <p className="text-xs text-gray-400 mb-1">Cobrado</p>
            <p className="text-lg font-bold text-accent-green">{fmt(pagado)}</p>
          </div>
          <div className="bg-surface-700/60 border border-red-500/20 rounded-xl p-3">
            <p className="text-xs text-gray-400 mb-1">Pendiente</p>
            <p className={`text-lg font-bold ${v.saldo_pendiente > 0 ? 'text-red-400' : 'text-gray-500'}`}>
        ...
        <div className="overflow-y-auto flex-1 space-y-3">
          <div className="card overflow-hidden">
            <table className="w-full text-xs">
              ...
              <tbody>
                {(v.detalles || []).map((d, i) => (
                  <tr key={i}>
                    <td className="text-gray-200">{d.nombre}</td>
                    <td className="text-center text-gray-400">{d.cantidad}</td>
                    <td className="text-right text-gray-400">{fmt(d.precio_unitario)}</td>
                    <td className="text-right font-semibold text-white">{fmt(d.subtotal)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {([...(v.pagos || []), ...(v.abonos || [])]).length > 0 && (
            <div>
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-2">Registro de Pagos</p>
              <div className="space-y-2">
                {[...(v.pagos || []), ...(v.abonos || [])].map((p, i) => (
                  <div key={i} className="flex justify-between items-center bg-surface-700/50 border border-white/5 px-4 py-2.5 rounded-xl">
    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-lg" style={{ maxHeight: '88vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="flex items-start justify-between mb-4 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-white capitalize">{fechaDisplay}</h2>
            <p className="text-sm text-gray-500 mt-0.5">{ventas.length} venta{ventas.length !== 1 ? 's' : ''} registrada{ventas.length !== 1 ? 's' : ''}</p>
          </div>
          <button onClick={onClose} className="btn-ghost btn-sm text-xl text-gray-400 hover:text-white">✕</button>
        </div>
        ...
        {!loading && (
          <div className="grid grid-cols-3 gap-3 mb-4 shrink-0">
            <div className="bg-surface-700/60 border border-accent-green/20 rounded-xl p-3">
              <p className="text-xs text-gray-400 mb-1">Ingreso Bruto</p>
              <p className="text-lg font-bold text-accent-green">{fmt(totalIngresos)}</p>
            </div>
            <div className="bg-surface-700/60 border border-brand-500/20 rounded-xl p-3">
              <p className="text-xs text-gray-400 mb-1">Cobrado</p>
              <p className="text-lg font-bold text-brand-300">{fmt(cobrado)}</p>
            </div>
            <div className="bg-surface-700/60 border border-red-500/20 rounded-xl p-3">
              <p className="text-xs text-gray-400 mb-1">Por Cobrar</p>
              <p className={`text-lg font-bold ${totalPendiente > 0 ? 'text-red-400' : 'text-gray-500'}`}>{fmt(totalPendiente)}</p>
            </div>
          </div>
        )}
        ...
        <div className="overflow-y-auto flex-1 card table-wrapper">
          {loading ? (
            ...
            <p className="text-center py-16 text-gray-500">No hay ventas para esta fecha</p>
          ) : (
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="stat-card">
          ...
          <p className="text-2xl font-bold text-white">{totalVentasMes}</p>
          <p className="text-xs text-gray-500 mt-1">en {diasConVentas} días activos</p>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          ...
          <p className="text-xl font-bold text-white">{fmt(diasConVentas > 0 ? totalMes / diasConVentas : 0)}</p>
        </div>
        <div className="stat-card">
          ...
          {mejorDia ? (
            <>
              ...
              <p className="text-xs text-gray-500 mt-1">{mejorDia.fecha}</p>
            </>
      ...
      <div className="card p-4 sm:p-5">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <button onClick={prevMonth} className="btn-ghost btn-sm p-2 rounded-lg hover:bg-surface-700"><LuChevronLeft /></button>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white capitalize">
                {format(viewDate, 'MMMM yyyy', { locale: es })}
              </h2>
            </div>
            <button onClick={nextMonth} className="btn-ghost btn-sm p-2 rounded-lg hover:bg-surface-700"><LuChevronRight /></button>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={loadCalendario} className="btn-ghost btn-sm p-2 rounded-lg hover:bg-surface-700">
        ...
        <div className="grid grid-cols-7 mb-1">
          {DIAS_SEMANA.map(d => (
            <div key={d} className="text-center text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider py-2">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isCurrentDay ? 'ring-2 ring-brand-500 bg-brand-900/20' : hasData ? getHeatColor(intensity) : 'bg-surface-700/30 hover:bg-surface-700/50'}
                `}
              >
                {/* Day number */}
                <span className={`text-[11px] sm:text-xs font-bold self-end mb-auto
                  ${isCurrentDay ? 'text-brand-400' : isCurrentMonth ? 'text-gray-300' : 'text-gray-600'}
                `}>
                  {format(day, 'd')}
                </span>
        ...
        {/* Legend */}
        <div className="flex flex-wrap gap-4 mt-4 pt-3 border-t border-white/5 text-xs text-gray-500">
          <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-surface-700/40 ring-1 ring-white/10" /> Sin ventas</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex bg-surface-800 p-1 rounded-xl border border-white/5 shadow-inner">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${tab === 'historial' ? 'bg-brand-600 text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-surface-700'}`}
            >
              <LuList size={14} /> Historial
            </button>
            <button
              onClick={() => setTab('calendario')}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${tab === 'calendario' ? 'bg-brand-600 text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-surface-700'}`}
            >
