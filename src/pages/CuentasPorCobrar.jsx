...
function AbonoModal({ venta, onClose, onSave }) {
  ...
  const handleMonto = (field, val) => {
    setForm(p => {
      ...
      if (field === 'monto_ves' && val) next.monto_usd = (parseFloat(val) / tasa).toFixed(4)
      return next
    })
  }
    ...
    <div className="modal-backdrop" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div className="bg-surface-700 rounded-xl p-3 mb-4">
          ...
          <p className="text-sm text-gray-400">Saldo: <strong className="text-red-400">${Number(venta.saldo_pendiente_usd).toFixed(2)}</strong></p>
        </div>
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Monto USD</label>
              <input className="input" type="number" min="0.01" step="0.01" max={venta.saldo_pendiente_usd} required value={form.monto_usd} onChange={e=>handleMonto('monto_usd',e.target.value)} /></div>
    ...
    <div className="modal-backdrop" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-lg">
        ...
        <div className="grid grid-cols-3 gap-3 mb-4 text-sm">
          <div className="bg-surface-700 rounded-xl p-3"><p className="text-gray-400 text-xs">Total</p><p className="font-bold text-white">{fmt(data.total_usd)}</p></div>
          <div className="bg-surface-700 rounded-xl p-3"><p className="text-gray-400 text-xs">Pagado</p><p className="font-bold text-accent-green">{fmt(data.total_usd - data.saldo_pendiente_usd)}</p></div>
          <div className="bg-red-900/20 rounded-xl p-3 border border-red-500/30"><p className="text-gray-400 text-xs">Pendiente</p><p className="font-bold text-red-400">{fmt(data.saldo_pendiente_usd)}</p></div>
        </div>
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {[...(data.pagos||[]), ...(data.abonos||[])].sort((a,b)=>a.fecha>b.fecha?1:-1).map((p,i)=>(
            <div key={i} className="flex justify-between text-sm bg-surface-700 rounded-lg px-3 py-2">
              <div><span className="text-white">{METODO_LABEL[p.metodo]||p.metodo}</span><span className="text-gray-500 ml-2 text-xs">{p.fecha?.slice(0,16)}</span></div>
              <span className="text-accent-green font-semibold">{fmt(p.monto_usd)}</span>
            </div>
          ))}
        </div>
